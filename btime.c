...
void print_time(struct timeval *elapsed, struct rusage *usage) {
  if (elapsed == NULL) {} else {
    printf("%ld.%06d,", elapsed->tv_sec, (int)elapsed->tv_usec);
    printf(" %ld.%06d,", usage->ru_utime.tv_sec, (int)usage->ru_utime.tv_usec);
    printf(" %ld.%06d,", usage->ru_stime.tv_sec, (int)usage->ru_stime.tv_usec);
    printf(" %ld\n", usage->ru_maxrss);
  }
}
